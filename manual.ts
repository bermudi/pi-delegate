...
export function getSubagentManualMarkdown(
  ...
  return [
    ...
    "```",
    "",
    "Delegate subagents to execute tasks in parallel. Each subagent gets an independent conversation. A batch with multiple write-capable tasks in one shared tree is rejected before dispatch; run them sequentially in separate calls or use disposable scratch workspaces.",
    "",
    ...
    '- Shared writers that overlap tasks in the same call or a running sync/async dispatch are rejected. Unknown tools count as mutating. Run them sequentially or use `workspace: "scratch"` when changes may be discarded.',
