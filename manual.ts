...
export function getSubagentManualMarkdown(
  ...
  return [
    ...
    "```",
    "",
    "Delegate subagents to execute tasks in parallel. Each subagent gets an independent conversation. A batch with multiple write-capable tasks in one shared tree is rejected before dispatch; run them sequentially in separate calls, use disposable scratch workspaces, or explicitly accept the risk with top-level `unsafeSharedWrites: true`.",
    "",
    ...
    "- Shared writers that overlap tasks in the same call or a running sync/async dispatch are rejected. Unknown tools count as mutating. `unsafeSharedWrites: true` bypasses this in-process check with no isolation or rollback.",
