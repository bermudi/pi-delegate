...
export function getSubagentManualMarkdown(
  ...
  return [
    ...
    '- `delegate({ action: "wait", ticket: "abc123", timeoutMs: 600_000 })` \u2014 block until finished or timeout',
    ...
    "## Config",
    "",
    "Tunables live in `~/.pi/agent/delegate.json`: `maxConcurrent` (sync ceiling), `maxAsyncTickets` (background ticket cap), per-model/per-provider concurrency limits, and per-agent model overrides.",
    "",
