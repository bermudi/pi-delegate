...
export function getSubagentManualMarkdown(
  ...
  return [
    ...
    '- `delegate({ action: "wait", ticket: "abc123", timeoutMs: 600_000 })` \u2014 block until the ticket finishes or the timeout expires',
    '- `delegate({ action: "cancel", ticket: "abc123" })` \u2014 preview activity and partial-write risk before cancelling',
    '- `delegate({ action: "cancel", ticket: "abc123", force: true })` \u2014 abort a running ticket after review',
    "",
    `Max ${getMaxAsyncTickets()} concurrent async tickets. Results are delivered automatically when all tasks finish. Use wait to block without consuming model turns, or poll for progress while doing other work.`,
    "A wait timeout returns the current running status and does not cancel the ticket.",
    "",
    "## Gotchas",
    "",
