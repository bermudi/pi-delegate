...
insert-only-on-success, stats-on-hit, per-sessionId serialization, TTL eviction).
...
`close`, `sweep`, `list`, `withSessionLock`) over a **private** `Map`; the raw
...
primitive that the caller brackets the whole run with; time is an internal seam
via a module-scoped `_setClockForTesting` override.
...
dependencies are language primitives (a `Map`, a promise-chain lock, a clock with
a test stand-in) — in-process + local-substitutable, no port at the external seam.
...
- Tests no longer hand-build `PooledAgent` literals or call `Map` methods; they
  go through `commit` / `configFor` and fake time via `_setClockForTesting`.
