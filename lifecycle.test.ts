...
describe("delegate task lifecycle integration", () => {
	...
	test("fresh task (no sessionId, no resumeFrom) creates agent and returns output", async () => {
		...
		const details = (result as any).details as {
			results: Array<{ output?: string; error?: string; agent?: string; tokens?: number }>;
		};
	...
	test("fresh task with systemPrompt override", async () => {
		...
		const result = await toolDef.execute(
			"tc-sysprompt-1",
			{ tasks: [{ prompt: "review code", systemPrompt: "You are a code reviewer." }] },
			undefined,
			undefined,
			ctx,
		);
	...
	test("parent model with no auth falls back to available alternative", async () => {
		...
		reg.getAvailable = () => [harnessModel, altModel].filter((m) =>
	...
	test("close action tears down pooled session", async () => {
		...
		expect((listBefore as any).details.results[0]?.output).toContain("close-me");
		...
		expect((listAfter as any).details.results[0]?.output).not.toContain("close-me");
	});
	...
	test("async task completes and results are pollable", async () => {
		...
		for (let i = 0; i < 50; i++) {
			...
			const details = pollResult.details as { status?: string; results?: Array<{ error?: string }> };
	...
	test("resumeFrom rehydrates from a previous session file", async () => {
		...
		const lines = [
			...
			JSON.stringify({ type: "message", id: "msg-1", parentId: null, timestamp: new Date().toISOString(), message: { role: "user", content: [{ type: "text", text: "Previous task" }], timestamp: Date.now() } }),
			JSON.stringify({ type: "message", id: "msg-2", parentId: "msg-1", timestamp: new Date().toISOString(), message: { role: "assistant", content: [{ type: "text", text: "Previous response." }], api: "openai-responses", provider: "test", model: "mock", usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "stop", timestamp: Date.now() } }),
		];
		...
		const result = await toolDef.execute(
			"tc-resume-1",
			{ tasks: [{ prompt: "continue from where you left off", resumeFrom: sessionFile }] },
			undefined,
			undefined,
			ctx,
		);
	...
	test("resumeFrom with nonexistent file returns error", async () => {
		...
		const result = await toolDef.execute(
			"tc-resume-404",
			{ tasks: [{ prompt: "resume", resumeFrom: "/nonexistent/path/session.jsonl" }] },
			undefined,
			undefined,
			ctx,
		);
...
function installFailingThenSuccess(failCount: number, failMessage: string, successText: string) {
	let callCount = 0;
	mock.module("@mariozechner/pi-ai", (orig) => ({
		...orig,
		streamSimple: () => {
...
describe("delegate retry and error recovery", () => {
	...
	test("transient error → retry → success", async () => {
		installFailingThenSuccess(1, "connection refused", "Recovered successfully.");
	...
	test("rate-limit error → retry → success", async () => {
		installFailingThenSuccess(1, "rate limit exceeded, try again later", "Rate limit cleared.");
...
describe("delegate pool-miss with resumeFrom and sessionId", () => {
	...
	test("sessionId not in pool + resumeFrom set → resumes and pools for reuse", async () => {
		...
		const lines = [
			...
			JSON.stringify({ type: "message", id: "msg-2", parentId: "msg-1", timestamp: new Date().toISOString(), message: { role: "assistant", content: [{ type: "text", text: "Original response." }], api: "openai-responses", provider: "test", model: "mock", usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, totalTokens: 0, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "stop", timestamp: Date.now() } }),
		];
		fs.writeFileSync(sessionFile, lines.join("\n") + "\n");
		...
		const r1 = await toolDef.execute(
			"tc-pool-resume-1",
			{ tasks: [{ prompt: "continue from resume", sessionId: "new-pooled-session", resumeFrom: sessionFile }] },
			undefined,
			undefined,
			ctx,
		);
		...
		const r2 = await toolDef.execute(
			"tc-pool-resume-2",
			{ tasks: [{ prompt: "continue in pool", sessionId: "new-pooled-session" }] },
			undefined,
			undefined,
			ctx,
		);
