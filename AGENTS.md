...
- **`telemetry.ts`** — local SQLite usage/health store. One `calls` row per `execute()` return path (sync finish, async spawn + settle, validation/manual/poll/cancel/wait short-circuits, host-compat failure) and one `tasks` row per completed `runResolvedTask`. Fail-open: Bun has no `node:sqlite`, so it silently no-ops in tests; Node 26+ uses `DatabaseSync` with WAL and a 5s busy timeout. Configured via `delegate.json` `telemetry`.
...
- **Built-in agents:** `default`, `scout`, `coder`, and `reviewer` are reserved and cannot be shadowed by Markdown profiles. `default` mirrors the live parent's exact model object, thinking level, delegatable native tools, and sanitized base prompt and bypasses delegate/settings model overrides. `scout` is read-only (`read`/`grep`/`find`/`ls`), `coder` is shared-workspace implementation (`read`/`write`/`edit`/`bash`), and `reviewer` runs `read`/`bash` in scratch by default. Fresh non-default built-ins inherit the exact parent model/thinking unless task or settings overrides win; reviewer can opt into shared persistence explicitly.
- **Custom agents are discovered from Markdown files.** `discoverAgents()` walks sources in order, first definition wins: project `.pi/agents/` → global `~/.pi/agent/agents/` → legacy `~/.agents/` → project `.claude/agents/` → global `~/.claude/agents`. A same-named `.md` in a higher-priority dir supersedes anything below it. Custom agents can also be defined inline in a task; Markdown agents are examples of custom agents. Reserved built-in names in persisted profiles are ignored with a warning.
...
- **Thinking precedence** — for fresh built-ins: task `thinking` > exact parent-model override (`settings.json` `delegate.agentOverridesByParentModel`) > unconditional agent override > a `:level` model suffix > parent thinking. Legacy `delegate.json` model maps remain for custom agents and do not replace built-in parent inheritance. Other named agents retain the existing frontmatter/frozen-pool behavior. The suffix is lowest on purpose, and an ignored suffix emits a warning.
- **Tools surface** — delegatable set is the 7 native tools (`read`/`write`/`edit`/`bash`/`grep`/`find`/`ls`) rebuilt from `TOOL_FACTORIES`; MCP/skills aren't tools and aren't inherited. `*`=read+write+edit+bash, `ro`=read+grep+find+ls (no shell). `bash` subsumes mutation and transitively unlocks MCP (mcporter), and shared workspaces have **no filesystem isolation**, so any set with `bash` is full-capability — "read-only" is honest only for `ro`.
- **Scratch workspaces** — per-task `workspace: "scratch"` creates an adjacent GNU `cp --archive --reflink=always` copy of the containing Git root (or cwd outside Git), translates a nested task cwd into it, runs the one-shot subagent there, maps reported touched paths back to the source tree, and deletes the copy after session quiescence. It never merges changes. This protects against ordinary relative writes; it is **not a security boundary** because absolute paths and unrestricted bash can still reach the host. Scratch uses in-memory transcripts (never offers `resumeFrom`) and rejects `sessionId`, `resumeFrom`, session actions, linked Git worktrees (their `.git` file shares metadata), project symlinks targeting outside the copy, non-Linux hosts or hosts without `/proc/self/fd`, and filesystems without strict reflink support.
...
- **Settings:** `~/.pi/agent/settings.json` or project `.pi/settings.json` — `delegate.agentOverrides` provides unconditional per-agent model/thinking/tool overrides; per-agent skill filtering is unsupported and warned/rejected. `delegate.agentOverridesByParentModel` provides exact `provider/model-id` scoped overrides. Project fields take precedence, and the settings cache is cleared between delegate dispatches.
...
- **Subagents must never run the parent's extension inventory.** `noExtensions: true` is non-negotiable for the default path; the only sanctioned exception is a narrow provider-scoped allowlist via `additionalExtensionPaths` in `host.ts` (defaults currently: only `openai-codex` + `git:github.com/bermudi/manaflow-pi-codex`). The allowlist can be replaced (per-provider, not appended) in `delegate.json` via `providerExtensions`; empty arrays are ignored. Allowlisted sources are resolved from the user scope only, and missing sources fail closed.
